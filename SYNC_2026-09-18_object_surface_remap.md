...
本轮把修正后的二元对象表面重映射实验同步到 `https://github.com/Zhenhao526/language`。正式矩阵包含 9 个 seed、18 个 parent 和 216 个 child；三片独立 replay audit 均通过，最大重放误差为 0。首个内容提交为 `56f9a1b7bb8fef19c20d84608dbe50256da9b28f`，已用 `git ls-remote` 核验远端 `main`。`results/formal_20260918_corrected/` 保留冻结配置、聚合指标、parent identity/swap shift、紧凑 per-run 结果、审计摘要和文件哈希；合并的完整 `results.json` 仅作为远端过程记录上传。
